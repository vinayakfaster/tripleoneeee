...
export default function Navbar({ currentUser, images, locationValue, title, listing, listingId }: Props) {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) setOpenDrop(null);
    };
  ...
  const handleEnquire = () => {
    const id = listingId || listingIds[Math.floor(Math.random() * listingIds.length)];
  ...
  const formatDate = (d?: Date) => d ? d.toLocaleDateString("en-IN", { day: "numeric", month: "short" }) : null;
  ...
  const DropUp = ({ children, minW = "220px" }: { children: React.ReactNode; minW?: string }) => (
    <div ref={dropdownRef} onClick={e => e.stopPropagation()} style={{
      position: "absolute", bottom: "calc(100% + 10px)", left: 0, minWidth: minW,
      background: OL[800], border: `1px solid ${OL.goldLight}`, borderRadius: "12px",
      overflow: "hidden", zIndex: 200, boxShadow: "0 -16px 48px rgba(0,0,0,0.55)",
    }}>
      {children}
    </div>
  );

  // icon color: transparent bg pe white, scrolled pe gold
  const iconColor = scrolled ? OL.gold : "rgba(255,255,255,0.9)";
    ...
    <>
      <style>{`
        ...
        .wa-btn {
          ...
          transition:transform 0.2s; flex-shrink:0; text-decoration:none;
        }
        ...
        .call-btn:hover { background:rgba(196,169,122,0.25); }
        ...
        .st-pill.active,.st-pill:hover {
          ...
          border-color:rgba(196,169,122,0.65); color:#f5f2ea;
        }
      ...
      {/* ══ ANNOUNCEMENT BAR ══ */}
      <div style={{
        position:"fixed", top:0, left:0, right:0, zIndex:3100, height:"32px",
        ...
        <div className="ol-marquee">
          {[...announcements,...announcements].map((a,i) => (
            <span key={i} style={{ fontFamily:"'Lato',sans-serif", fontSize:"10px", letterSpacing:"0.2em", color:OL.cream, paddingRight:"60px" }}>
              <span style={{ color:OL.gold, marginRight:"10px" }}>✦</span>{a}
            </span>
          ))}
        </div>
        <div style={{ position:"absolute", left:"14px", color:OL.gold, fontSize:"9px", letterSpacing:"0.3em", background:OL[900], paddingRight:"8px" }}>
          TRIPLEONE
        </div>
      </div>
      ...
      >
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 18px" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
            ...
            {/* 📞 Call icon — always visible, icon only, no text ever */}
            <a href="tel:+918448811165" className="call-btn" title="+91 8448811165">
              <svg width="16" height="16" fill={iconColor} viewBox="0 0 24 24">
                <path d="M6.62 10.79a15.05 15.05 0 006.59 6.59l2.2-2.2a1 1 0 011.01-.24 11.47 11.47 0 003.58.57 1 1 0 011 1V20a1 1 0 01-1 1A17 17 0 013 4a1 1 0 011-1h3.5a1 1 0 011 1 11.47 11.47 0 00.57 3.58 1 1 0 01-.25 1.01l-2.2 2.2z"/>
              </svg>
            </a>

            {/* 💬 WhatsApp blinking icon — always visible */}
            <a href="https://wa.me/918448811165" target="_blank" rel="noopener noreferrer"
              className="wa-btn" title="Chat on WhatsApp">
            ...
            {/* ENQUIRE */}
            <motion.button whileTap={{ scale:0.95 }} onClick={handleEnquire} style={{
            ...
            {currentUser && (
              <div onClick={() => setMenuOpen(true)} style={{ cursor:"pointer" }}>
                <Image src={currentUser.image || "/assets/avatar.png"} alt="avatar"
                  width={32} height={32}
      ...
      {/* ══ HERO ══ */}
      <div style={{
        position:"relative", width:"100vw", overflow:"hidden", height:"100vh",
        marginLeft:"calc(-50vw + 50%)", marginRight:"calc(-50vw + 50%)", marginTop:"32px",
      }}>
        <AnimatePresence mode="wait">
          <motion.div key={displayImages[index]}
            ...
            <Image src={displayImages[index]} alt="hero" fill priority quality={100} sizes="100vw" style={{ objectFit:"cover" }}/>
          </motion.div>
        </AnimatePresence>

        {/* Search + Pills — home only */}
        {!isListingPage && (
          <div style={{
            ...
            <motion.div initial={{ opacity:0,y:10 }} animate={{ opacity:1,y:0 }} transition={{ delay:0.4 }}
              style={{ display:"flex", alignItems:"center", background:"rgba(26,34,16,0.6)", backdropFilter:"blur(10px)", border:`1px solid rgba(196,169,122,0.18)`, borderRadius:"8px", overflow:"hidden" }}>
              {[
                ...
                <div key={i} style={{ display:"flex", alignItems:"center", gap:"6px", padding:"7px 12px", borderRight: i<arr.length-1 ? "1px solid rgba(196,169,122,0.12)" : "none" }}>
                  <span style={{ color:OL.gold, fontSize:"10px" }}>{u.icon}</span>
                  <span style={{ fontFamily:"'Lato',sans-serif", fontSize:"9px", letterSpacing:"0.1em", color:"rgba(245,242,234,0.6)", whiteSpace:"nowrap" }}>{u.text}</span>
                </div>
              ))}
            </motion.div>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:"7px", justifyContent:"center" }}>
              {stayTypes.map((st,i) => (
                <motion.button key={st.value}
                  ...
                  <span style={{ fontSize:"12px" }}>{st.icon}</span>{st.label}
                </motion.button>
              ))}
            </div>
            ...
            <motion.div initial={{ opacity:0,y:6 }} animate={{ opacity:1,y:0 }} transition={{ delay:0.2 }}
              ...
              style={{ display:"inline-flex", alignItems:"center", gap:"10px", background:"rgba(45,58,30,0.7)", backdropFilter:"blur(12px)", border:`1px solid ${OL.goldLight}`, borderRadius:"50px", padding:"7px 18px", cursor:"pointer" }}>
              ...
              <span style={{ background:OL[500], color:OL.cream, fontSize:"9px", fontWeight:700, letterSpacing:"0.1em", padding:"2px 10px", borderRadius:"50px" }}>CLAIM</span>
            </motion.div>
            ...
            <div onClick={e => e.stopPropagation()} style={{
              ...
              overflow:"visible", position:"relative", display:"flex", alignItems:"stretch",
            }}>
              {/* Stay Type */}
              <div className="sr-field" style={{ minWidth: isMobile ? "110px" : "150px", position:"relative" }}
                ...
                {openDrop==="type" && (
                  <DropUp>
                    {stayTypes.map(st => (
                      <div key={st.value} onClick={() => { setSelectedType(st.value); setOpenDrop(null); }}
                        style={{ display:"flex", alignItems:"center", gap:"12px", padding:"11px 16px", cursor:"pointer", borderBottom:"1px solid rgba(196,169,122,0.08)", background: selectedType===st.value ? "rgba(196,169,122,0.1)" : "transparent", transition:"background 0.15s" }}
              ...
              <div className="sr-field" style={{ position:"relative" }}
                ...
                <div className={`sr-val ${!selectedLoc ? "sr-placeholder" : ""}`}>{selectedLoc?.label || "Where?"}</div>
                {openDrop==="loc" && (
                  <DropUp minW="200px">
                    <div style={{ padding:"10px 12px", borderBottom:`1px solid ${OL.goldLight}` }} onClick={e => e.stopPropagation()}>
                      <input autoFocus value={locSearch} onChange={e => setLocSearch(e.target.value)} placeholder="Search city..."
                        style={{ width:"100%", background:"rgba(61,82,40,0.5)", border:`1px solid ${OL.goldLight}`, borderRadius:"8px", padding:"8px 12px", color:OL.cream, fontFamily:"'Lato',sans-serif", fontSize:"12px", outline:"none" }}/>
                    </div>
                    {filteredLocs.map(loc => (
                      <div key={loc.value} onClick={() => { setSelectedLoc(loc); setOpenDrop(null); setLocSearch(""); }}
                        style={{ display:"flex", alignItems:"center", gap:"10px", padding:"11px 16px", cursor:"pointer", borderBottom:"1px solid rgba(196,169,122,0.08)", transition:"background 0.15s" }}
                        onMouseEnter={e => (e.currentTarget.style.background="rgba(196,169,122,0.1)")}
              ...
              <div className="sr-field" style={{ position:"relative" }}
                ...
                <div className={`sr-val ${!dateRange.startDate ? "sr-placeholder" : ""}`}>{formatDate(dateRange.startDate) || "Add date"}</div>
              </div>
              ...
              <div className="sr-field hidden md:flex" style={{ flexDirection:"column" }}
                ...
                <div className={`sr-val ${!dateRange.endDate ? "sr-placeholder" : ""}`}>{formatDate(dateRange.endDate) || "Add date"}</div>
              </div>
              ...
              {openDrop==="cal" && (
                <div ref={dropdownRef} onClick={e => e.stopPropagation()} style={{
                  ...
                  left: isMobile ? "-16px" : "30%", zIndex:200,
                  background:OL[800], border:`1px solid ${OL.goldLight}`,
                  borderRadius:"12px", overflow:"hidden", boxShadow:"0 -16px 48px rgba(0,0,0,0.55)",
                }}>
                  <Calendar value={dateRange} onChange={({ selection }: any) => {
              ...
              <button onClick={handleSearch} style={{
                flexShrink:0, background:`linear-gradient(135deg,${OL[500]},${OL[600]})`,
                border:"none", cursor:"pointer", padding: isMobile ? "0 14px" : "0 24px",
                display:"flex", alignItems:"center", justifyContent:"center",
                gap:"6px", borderRadius:"0 10px 10px 0", transition:"all 0.2s", minHeight:"52px",
              }}
              ...
              >
                ...
                <span className="hidden md:block" style={{ fontFamily:"'Lato',sans-serif", fontSize:"10px", fontWeight:700, letterSpacing:"0.18em", color:OL.cream }}>SEARCH</span>
              </button>
            </div>
          </div>
        )}
        ...
        {isListingPage && (
          <div style={{ position:"absolute", bottom:"9rem", left:0, right:0, zIndex:20, textAlign:"center", padding:"0 16px" }}>
            {title && (
              <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(28px,5vw,52px)", fontWeight:400, color:"#fff", letterSpacing:"0.05em", marginBottom:"12px", textShadow:"0 2px 20px rgba(0,0,0,0.6)" }}>{title}</h1>
            )}
            {locationValue && (
              <div style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"rgba(26,34,16,0.6)", backdropFilter:"blur(8px)", border:`1px solid ${OL.goldLight}`, color:OL.cream, padding:"6px 18px", borderRadius:"50px", fontSize:"13px", fontFamily:"'Lato',sans-serif" }}>
        ...
        {displayImages.length > 1 && (
          <div style={{ position:"absolute", right:"14px", top:"50%", transform:"translateY(-50%)", zIndex:20, display:"flex", flexDirection:"column", gap:"6px" }}>
            {displayImages.map((_,i) => (
              <button key={i} onClick={() => setIndex(i)} style={{
                width: i===index ? "6px" : "5px", height: i===index ? "22px" : "5px",
                borderRadius:"3px", background: i===index ? OL.gold : "rgba(255,255,255,0.22)",
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            <motion.div
              style={{ background:OL[800], width:"280px", height:"100%", padding:"24px", display:"flex", flexDirection:"column", borderRight:`1px solid ${OL.goldLight}`, boxShadow:"4px 0 40px rgba(0,0,0,0.5)", overflowY:"auto" }}
              ...
              <button onClick={() => setMenuOpen(false)}
                style={{ alignSelf:"flex-end", background:"none", border:"none", color:OL.gold, fontSize:"20px", cursor:"pointer" }}>✕</button>

              <div style={{ height:"1px", background:`linear-gradient(to right,${OL.gold},transparent)`, margin:"10px 0 18px" }}/>
              <div onClick={() => { router.push("/"); setMenuOpen(false); }} style={{ cursor:"pointer" }}>
                <Logo/>
              </div>
              ...
              {[
                ...
                <span key={label} className="menu-item" onClick={() => { router.push(path); setMenuOpen(false); }}>{label}</span>
              ))}

              <span className="menu-item" onClick={() => { handleBecomeHost(); setMenuOpen(false); }} style={{ color:OL.gold }}>
                🏡 Become a Host
              </span>

              <div style={{ height:"1px", background:"rgba(196,169,122,0.2)", margin:"16px 0" }}/>
              <p style={{ fontFamily:"'Lato',sans-serif", fontSize:"9px", letterSpacing:"0.3em", color:OL.gold, textTransform:"uppercase", marginBottom:"10px" }}>Account</p>

              {!currentUser ? (
                <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
                  <button onClick={() => { loginModal.onOpen(); setMenuOpen(false); }}
                    style={{ width:"100%", padding:"12px", background:`linear-gradient(135deg,${OL[500]},${OL[600]})`, border:`1px solid ${OL.goldLight}`, color:OL.cream, fontFamily:"'Lato',sans-serif", fontSize:"11px", fontWeight:700, letterSpacing:"0.15em", textTransform:"uppercase", cursor:"pointer", borderRadius:"4px" }}>
                    Login
                  </button>
                  <button onClick={() => { registerModal.onOpen(); setMenuOpen(false); }}
                    style={{ width:"100%", padding:"12px", background:"transparent", border:`1px solid ${OL.goldLight}`, color:OL.gold, fontFamily:"'Lato',sans-serif", fontSize:"11px", fontWeight:700, letterSpacing:"0.15em", textTransform:"uppercase", cursor:"pointer", borderRadius:"4px" }}>
                    Sign Up
                  </button>
                </div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"14px" }}>
                    <Image src={currentUser.image || "/assets/avatar.png"} alt="avatar" width={36} height={36}
                      style={{ borderRadius:"50%", border:`2px solid ${OL.gold}` }}/>
                    <div>
                      <p style={{ fontFamily:"'Lato',sans-serif", fontSize:"13px", color:OL.cream, fontWeight:600 }}>{currentUser.name}</p>
                      <p style={{ fontFamily:"'Lato',sans-serif", fontSize:"10px", color:OL.gold }}>{currentUser.email}</p>
                    </div>
                  </div>
                  {[
                    ...
                    <div key={label} onClick={() => { router.push(path); setMenuOpen(false); }}
                      style={{ display:"flex", alignItems:"center", gap:"10px", padding:"12px 0", cursor:"pointer", borderBottom:"1px solid rgba(196,169,122,0.08)", fontFamily:"'Lato',sans-serif", fontSize:"12px", color:OL.cream, letterSpacing:"0.05em", transition:"color 0.2s" }}
                  ...
                  <button onClick={() => { signOut(); setMenuOpen(false); }}
                    style={{ marginTop:"12px", width:"100%", padding:"10px", background:"transparent", border:"1px solid rgba(192,57,43,0.4)", color:"#e74c3c", fontFamily:"'Lato',sans-serif", fontSize:"11px", letterSpacing:"0.15em", textTransform:"uppercase", cursor:"pointer", borderRadius:"4px" }}>
                    🚪 Logout
                  </button>
                </div>
              )}
              ...
              <div style={{ marginTop:"auto", paddingTop:"20px", borderTop:`1px solid ${OL.goldLight}` }}>
                <a href="tel:+918448811165" style={{ display:"flex", alignItems:"center", gap:"8px", color:OL.gold, fontSize:"12px", fontFamily:"'Lato',sans-serif", textDecoration:"none" }}>
